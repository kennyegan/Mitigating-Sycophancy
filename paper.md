...
**GSM8k evaluation methodology.** GSM8k capability scores throughout this paper use strict normalized numeric equality on model-generated completions. The answer extraction pipeline parses the model's chain-of-thought output for a final numeric answer and compares it against the ground truth after normalization (stripping whitespace, commas, trailing zeros, and currency symbols). An earlier version of the extraction regex (used in the preliminary March 3 archive snapshot) underperformed on Llama-3's generation format, yielding 11.3% accuracy on the same N=1,319 test set that the corrected pipeline scores at 33.2%. All GSM8k numbers reported in this paper use the corrected extraction pipeline, validated across all experiments.

---

## 5. Results

**Note:** All values are sourced from the validated rerun artifacts in `results/`, confirmed by `results/full_rerun_manifest.json` (`missing_count: 0`). An earlier snapshot (`results_archive/results_20260303T225104Z/`) contains preliminary results from the first half of the pipeline; the canonical results reflect the complete validated rerun with corrected GSM8k answer extraction (see Section 4). GSM8k capability scores use strict normalized numeric equality on generated completions. DPO results from `results/dpo_eval_results.json` (Apr 7, 2026).
...
**Effect sizes (Cohen's h for proportions; pp = percentage points throughout):**
- Opinion vs. Reasoning: h = 2.276 (very large)
- Opinion vs. Factual: h = 2.022 (very large)
- Reasoning vs. Factual: h = −0.254 (small)

Note: The large h values (>2.0) are partly dominated by floor effects when comparing against 0% rates. Cohen's d on the underlying continuous compliance gap distributions provides a complementary perspective: d = 0.18 (opinion vs. reasoning), d = 0.78 (opinion vs. factual), indicating that while the proportional differences are extreme, the distributional separation on the continuous compliance measure is moderate for opinion vs. reasoning and large for opinion vs. factual.
...
Logistic regression probes were trained at each of the 32 residual stream layers using the **neutral-transfer** design: probes trained exclusively on neutral prompt activations and tested on biased prompt activations from the same samples (see **Figure 4** for accuracy curves across layers using the balanced dataset with randomized answer positions). This design prevents the probe from learning prompt-format cues (e.g., "I think..." preambles in biased prompts) and instead tests whether it tracks a format-invariant truth direction. Labels encode the correct answer identity (lexicographic ordering of answer options). 5-fold StratifiedKFold cross-validation, 1,500 samples.
...
| Rank | Head | Mean Recovery | Std Dev |
|------|------|---------------|---------|
| 1 | **L4H28** | 0.4428 | 0.550 |
| 2 | **L4H5** | 0.3020 | 0.537 |
| 3 | **L5H31** | 0.2564 | 0.549 |
| 4 | L2H5 | 0.2445 | 0.366 |
| 5 | L3H30 | 0.2182 | 0.413 |
| 6 | L5H24 | 0.1685 | 0.250 |
| 7 | L3H17 | 0.1605 | 0.310 |
| 8 | L3H28 | 0.1548 | 0.273 |
| 9 | L1H11 | 0.1435 | 0.277 |
...
**Note:** Mean-ablation of all 3 heads caused catastrophic output degradation (all 1,500 samples produced unparseable outputs), yielding 0% on all metrics. This is excluded from analysis. One hypothesis: replacing head outputs with dataset-mean activations injects a coherent but contextually inappropriate signal into the residual stream, disrupting downstream computation more severely than zero ablation (which simply removes the heads' contribution). The network appears more robust to missing information than to misleading information, consistent with self-repair findings (McGrath et al., 2023).
...
**Key finding:** Even ablating all 10 patching-identified heads simultaneously produces **no meaningful sycophancy reduction** (+0.5 pp, within sampling noise). A two-proportion z-test confirms the +0.5pp difference is not statistically significant (z=0.28, p=0.78, 95% CI for the difference: [−2.9pp, +3.9pp]). At N=1,500 and α=0.05, a two-proportion z-test (both groups estimated) has 80% power to detect effects of approximately ±4.5 pp (MDE = (z_{0.975} + z_{0.80}) × √(2p(1−p)/N) with p=0.28), confirming that the null is informative: we can rule out sycophancy reductions larger than ~4.5 pp from top-10 ablation. MMLU is preserved and slightly improves. GSM8k shows a modest non-significant decrease (90.0% retained). This confirms that the sycophantic behavior is **redundantly distributed** across the network — the patching-identified circuit captures activation patterns correlated with sycophancy, but the behavior is not causally dependent on these specific heads.
...
Note: Post-DPO, the best-transfer layer shifts from Layer 1 to Layer 4 (88.3% transfer accuracy). We report Layer 1 for direct comparability with the pre-DPO analysis. Layer 4 results show qualitatively identical but larger effects (robust tracking +21.9 pp).
...
The pattern is consistent across all tested layers (0–5): social compliance decreases by 5.7–6.7 pp, robust tracking increases by 15.0–27.1 pp, and belief corruption changes minimally (−1.6 to −2.7 pp). Fisher's exact test confirms the social compliance reduction (18.0% → 11.4%, p < 0.001) and robust tracking increase (59.9% → 75.5%, p < 0.001) are statistically significant.
...
**Targeted fine-tuning vs. ablation.** Chen et al. (2024) achieve substantial sycophancy reduction via Supervised Pinpoint Tuning (SPT) — targeted fine-tuning of gradient/activation-identified modules — in Llama-2-Chat, while our head ablation produces a null result in Llama-3-8B-Instruct. Several factors may explain this divergence: (a) Chen et al. use gradient/activation-based module selection, which identifies more specific computational targets than our node-level residual stream patching; (b) their intervention is targeted fine-tuning rather than ablation — fine-tuning reshapes rather than removes components, which may overcome redundancy by modifying all relevant pathways simultaneously; (c) they study challenge-induced sycophancy (model reverses a correct answer under user pushback), while we study assertion-based sycophancy (model agrees with an initial user opinion) — these may engage different circuits with different redundancy properties; (d) Llama-2 and Llama-3 have substantially different RLHF procedures that may produce different levels of circuit redundancy. The success of Chen et al.'s targeted fine-tuning is consistent with our own DPO finding: training-time methods that reshape the circuit succeed where inference-time ablation fails.
...
The DPO probe re-analysis (Section 5.11) resolves a key open question: does training-time sycophancy reduction reflect a change in internal representations, or only in output behavior? The answer is unambiguous: DPO specifically eliminates **social compliance** (−6.6 pp) while leaving belief corruption essentially unchanged (−1.7 pp). The 15.6 pp increase in robust tracking means DPO strengthens the connection between the model's internal truth representations and its output, without altering the truth representations themselves.

...
Note: The large h values (>2.0) are partly dominated by floor effects when comparing against 0% rates. Cohen's d on the underlying continuous compliance gap distributions provides a complementary perspective: d = 0.18 (opinion vs. factual), d = 0.78 (opinion vs. reasoning), indicating that while the proportional differences are extreme, the distributional separation on the continuous compliance measure is moderate for opinion vs. factual and large for opinion vs. reasoning.

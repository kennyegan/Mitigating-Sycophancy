...
GSM8k accuracy & 33.2\% & 38.5\% & +5.3\,pp$^\dagger$ \\
\bottomrule
\multicolumn{4}{l}{\footnotesize $^\dagger$Post-DPO GSM8k evaluated on $N{=}200$ vs.\ baseline $N{=}1{,}319$.}\\
\multicolumn{4}{l}{\footnotesize 95\% CI [32.0\%, 45.4\%] overlaps baseline; improvement not significant.}\\
\multicolumn{4}{l}{\footnotesize MMLU uses $N{=}500$ for both pre- and post-DPO (consistent comparison).}
\end{tabular}
\end{table}
...
Sections~\ref{sec:circuit}--\ref{sec:steering} establish that inference-time methods cannot selectively reduce sycophancy due to circuit redundancy. DPO succeeds where they fail (Table~\ref{tab:dpo_behavioral}): opinion sycophancy drops 23.8\,pp while MMLU is preserved (+0.8\,pp) and GSM8k is preserved (38.5\%, $N{=}200$, 95\% CI [32.0\%, 45.4\%] overlapping the $N{=}1{,}319$ baseline of 33.2\%). The intervention is domain-specific---factual and reasoning sycophancy, already near zero, remain unaffected. \textbf{Generalization caveat:} these results are in-distribution (evaluation uses the same prompt format and source as training); out-of-distribution transfer remains untested.
...
\begin{enumerate}
    ...
    \item \textbf{DPO reduces opinion sycophancy by 23.8\,pp while preserving capabilities.} MMLU +0.8\,pp; GSM8k preserved (38.5\%, $N{=}200$, 95\% CI overlapping baseline). Training-time intervention succeeds where inference-time methods fail.
